...
图像生成与编辑使用独立的接口：`runImage()` 按顺序尝试一组图像模型，支持单模型重试、降级、出图校验，并为每次服务商调用留存记录；内置 OpenAI Images API、OpenRouter 与火山方舟 Seedream 的原生 adapter。详见[图像生成](https://github.com/open-multi-agent/open-multi-agent/blob/main/docs/image-generation.md)。
