...
任务 DAG 默认按事件驱动执行：下游任务在依赖满足时立即启动，不再等待同一
ready set 中无关的任务。不同分支的 progress 事件因此可以交错出现，而不再
按轮成批到达。审批兼容模式与自建 UI 的迁移方法见
...
原有四种策略在逐个处理 ready task 时保留各自的选择契约。`composite` 使用
`schedulingWeights.fit` 与 `schedulingWeights.load`，缺省分别为 `0.7` 和
`0.3`。当前负载是在调度时按 roster 归一化的 `in_progress` 任务数；同一次
调用中先前完成的分配不会改变该快照。若 `requires` 过滤后没有合格 Agent，
`capability-match` 会以 `NO_ELIGIBLE_AGENT` 终止；`composite` 则发出同名
结构化 `warning`，再显式回退到 fit 为零、只结合当前负载的路径。
