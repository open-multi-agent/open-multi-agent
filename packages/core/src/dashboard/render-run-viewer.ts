  ...
  <script>
    (function () {
      ...
      function renderDetails(item, type) {
        ...
        var overview = document.createElement('section'); overview.className = 'detail-section detail-grid'; overview.appendChild(detailPair('Kind', item.kind || type)); overview.appendChild(detailPair('Duration',fmtDuration(item.durationMs))); overview.appendChild(detailPair('Task',item.taskId || item.id)); overview.appendChild(detailPair('Agent',item.agent || item.assignee)); overview.appendChild(detailPair('Model',item.model)); overview.appendChild(detailPair('Provider',item.provider)); overview.appendChild(detailPair('Tool',item.tool)); overview.appendChild(detailPair('Retries',item.retries)); root.appendChild(overview);
