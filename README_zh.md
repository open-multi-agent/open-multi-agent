...
- `OPENAI_API_KEY`
- `OPENROUTER_API_KEY`（OpenRouter，走 OpenAI 兼容适配器）
- `GEMINI_API_KEY`
- `XAI_API_KEY`（Grok）
...
如果你在看 [Mastra](https://github.com/mastra-ai/mastra)：它是 Supervisor 模式，自己定义 agent、定义 workflow，再手动接 Supervisor。`open-multi-agent` 走的是另一条路，协调者把目标自动拆成任务 DAG，入口就是一句 `runTeam(team, "构建一个 REST API")`。Mastra 给你显式拓扑，这边给你目标驱动的自动编排。选哪个看任务适合哪种抽象。
