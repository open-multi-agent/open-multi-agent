/**
 * Multi-Agent Team Collaboration with ByteDance DouBao
 *
 * Three specialized agents (architect, developer, reviewer) collaborate via `runTeam()`
 * to build a minimal Express.js REST API. Every agent uses DouBao's flagship model.
 *
 * Run:
 *   npx tsx examples/providers/doubao.ts
 *
 * Prerequisites:
 *   DOUBAO_API_KEY environment variable must be set.
 *   DOUBAO_BASE_URL environment variable can be set to switch to a different endpoint if needed.
 *
 * Endpoints:
 *   Default (Beijing): https://ark.cn-beijing.volces.com/api/v3
 *   (Set DOUBAO_BASE_URL to use a different endpoint)
 *
 * Available models:
 *   Check your DouBao API documentation for available model names.
 *   Example: 'doubao-seed-2-0-lite-260428' (or any model available to your API key)
 */
...
if (!DOUBAO_API_KEY) {
  throw new Error('DOUBAO_API_KEY environment variable must be set.')
}

// ---------------------------------------------------------------------------
// Agent definitions (all using DouBao)
// ---------------------------------------------------------------------------
...
const goal = `Create a minimal Express.js REST API in /tmp/doubao-express-api/ with:
