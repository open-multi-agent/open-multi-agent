/**
 * Multi-Agent Team Collaboration with Zhipu GLM
 *
 ...
 *
 * Available models:
 *   glm-4-plus   — Zhipu flagship model (recommended for reasoning and coding tasks)
 *   glm-4-air    — Faster, cost-efficient model
 */
...
const OUTPUT_DIR = join(process.cwd(), '.agent-workspace', 'zhipu-api')

const ZHIPU_BASE_URL = 'https://open.bigmodel.cn/api/paas/v4'
const ZHIPU_API_KEY = process.env.ZHIPU_API_KEY

if (!ZHIPU_API_KEY) {
  throw new Error('ZHIPU_API_KEY environment variable must be set.')
}

// ---------------------------------------------------------------------------
// Agent definitions (all using Zhipu GLM via the OpenAI-compatible adapter)
// ---------------------------------------------------------------------------
