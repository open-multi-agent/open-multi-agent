/**
 * Multi-Agent Team Collaboration with Qwen (DashScope)
 *
 * Three specialized agents (architect, developer, reviewer) collaborate via `runTeam()`
 * to build a minimal Express.js REST API. Every agent uses Qwen through DashScope's
 ...
 *
 * Optional:
 *   QWEN_MODEL overrides the default model.
 */
...
if (!DASHSCOPE_API_KEY) {
  throw new Error('DASHSCOPE_API_KEY environment variable must be set.')
}

const qwenConfig = {
  provider: 'openai', // DashScope exposes an OpenAI-compatible Chat Completions API.
...
// ---------------------------------------------------------------------------
// Agent definitions (all using Qwen via the OpenAI-compatible adapter)
// ---------------------------------------------------------------------------
...
const orchestrator = new OpenMultiAgent({
  ...
  defaultApiKey: DASHSCOPE_API_KEY,
...
const goal = `Create a minimal Express.js REST API in /tmp/qwen-express-api/ with:
