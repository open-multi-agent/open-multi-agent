...
| Built-in image adapters (`OpenAIImageAdapter`, `OpenRouterImageAdapter`, `SeedreamImageAdapter`) | Enforced when `egressPolicy` is passed to the adapter. Every request is checked against the policy before it opens, and redirects are rejected. The adapters request base64 output and never fetch a returned image URL, so no second origin is contacted. A denial fails the attempt as a non-retryable `invalid_request`; see [image generation](image-generation.md). |
