...
| Built-in image adapters (`BlackForestLabsImageAdapter`, `OpenAIImageAdapter`, `OpenRouterImageAdapter`, `SeedreamImageAdapter`) | Enforced when `egressPolicy` is passed to the adapter. Every request is checked against the policy before it opens, and redirects are rejected. The OpenAI, OpenRouter, and Seedream adapters request base64 output and never fetch a returned image URL. The Black Forest Labs adapter polls and downloads from the URLs its API returns, and each of those requests is checked against the policy too. A denial fails the attempt as a non-retryable `invalid_request`; see [image generation](image-generation.md). |
